...
const ImageShareModal: React.FC<{
  ...
  const handleCopy = () => {
    // In a real app, this would be a public URL. For now, we'll copy the data URL.
...
const MessageComponent: React.FC<MessageProps> = ({ message, onUpdateMessage, onSendMessage, onRegenerateResponse, onAddTask }) => {
            ...
            return (
                <>
                    <button onClick={pause} className="p-1 text-slate-400 hover:text-white transition-transform hover:scale-110" aria-label="Pause reading">
                        <PauseIcon className="w-4 h-4 text-pink-400" />
                    </button>
  ...
  const renderContent = () => {
      ...
      return (
        <div className="w-full">
          <textarea
            ...
            className="w-full p-2 bg-slate-800 border border-slate-600 rounded-md focus:outline-none focus:ring-2 focus:ring-pink-500"
          ...
          <div className="flex justify-end gap-2 mt-2">
            ...
            <button onClick={handleSaveEdit} className="px-3 py-1 text-sm rounded-md bg-pink-600 hover:bg-pink-700">Save & Submit</button>
          </div>
        </div>
      );
    }
    ...
    return (
      <div className="prose prose-invert prose-sm break-words max-w-none prose-p:my-2 prose-pre:my-2 prose-ul:my-3 prose-ol:my-3 prose-li:my-1.5">
        {message.imageUrl && (
            <div className="my-2 relative group/image">
                <button onClick={() => setPreviewImageUrl(message.imageUrl)} className="block w-full">
                    <img src={message.imageUrl} alt={message.imagePrompt || 'Generated content'} className="rounded-lg max-w-full h-auto cursor-pointer" />
                </button>
                 <div className="absolute bottom-2 left-1/2 -translate-x-1/2 flex items-center gap-1 bg-black/40 backdrop-blur-sm p-1 rounded-full opacity-0 group-hover/image:opacity-100 transition-opacity duration-300">
                    <button onClick={() => setFeedback('like')} className={`p-2 rounded-full transition-colors ${feedback === 'like' ? 'text-green-400 bg-green-500/20' : 'text-slate-300 hover:bg-slate-700/50'}`} aria-label="Like">
                        <ThumbsUpIcon className="w-4 h-4" />
                    </button>
                    <button onClick={() => setFeedback('dislike')} className={`p-2 rounded-full transition-colors ${feedback === 'dislike' ? 'text-red-400 bg-red-500/20' : 'text-slate-300 hover:bg-slate-700/50'}`} aria-label="Dislike">
                        <ThumbsDownIcon className="w-4 h-4" />
                    </button>
                    <button onClick={() => handleDownload(message.imageUrl!, message.imagePrompt || 'generated-image')} className="p-2 text-slate-300 hover:bg-slate-700/50 rounded-full transition-colors" aria-label="Download">
                        <DownloadIcon className="w-4 h-4" />
                    </button>
                     <button onClick={() => setIsShareModalOpen(true)} className="p-2 text-slate-300 hover:bg-slate-700/50 rounded-full transition-colors" aria-label="Share">
    ...
    <>
      ...
      <div className={`group flex items-start gap-3 w-full animate-fadeInUp ${isUser ? 'justify-end' : 'justify-start'}`}>
        ...
        <div className="relative flex flex-col w-fit max-w-[85%] md:max-w-[75%]">
            <div className={`px-4 py-3 rounded-xl ${isUser ? 'bg-pink-600/80 backdrop-blur-sm text-white rounded-br-sm' : 'text-slate-200'}`}>
                {renderContent()}
            </div>
            <div className={`relative flex items-center gap-2 mt-1 transition-opacity opacity-0 group-hover:opacity-100 ${isUser ? 'justify-end pr-1' : 'pl-1'}`}>
                {!isUser && !hasFailed && !message.imageUrl && (
                  <>
                    ...
                    {message.videoUrl && (
                      <button onClick={() => handleDownload(message.videoUrl!, `fari-video.mp4`)} className="p-1 text-slate-400 hover:text-white transition-transform hover:scale-110" aria-label="Download video">
