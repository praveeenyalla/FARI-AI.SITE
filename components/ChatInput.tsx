...
const ChatInput: React.FC<ChatInputProps> = ({ onSendMessage, isSending, activeMode, onToggleVoiceMode, voiceEndedInfo, onClearVoiceEndedInfo, onModeChange, onDrawSketchClick }) => {
  ...
  return (
    ...
    <div className="w-full max-w-3xl mx-auto px-4 pb-3">
      ...
      >
        {attachment ? (
            <div className="flex-shrink-0 ml-2 mb-1.5 p-1.5 bg-pink-500/20 rounded-lg text-xs text-pink-300 flex items-center gap-2">
        ...
        <div className="flex items-center gap-1 pr-1 flex-shrink-0 self-center">
            ...
            <button 
              onClick={() => handleSend()} 
              disabled={!canSubmit} 
              className={`w-9 h-9 rounded-full text-white transition-all duration-200 flex items-center justify-center ${canSubmit ? 'bg-pink-600 hover:bg-pink-500' : 'bg-slate-700 opacity-50 cursor-not-allowed'}`} 
